...
- `document_symbol` parses on demand via `jotdown::Parser::new(text).into_offset_iter()`, which yields `(Event, Range<usize>)`. `heading_symbols()` builds a **nested** hierarchy: jotdown wraps each heading in a `Section` container that nests by level, so it walks the section `Start`/`End` events with a stack — the section span is the symbol `range`, the heading line is the `selection_range`, and nested sections become `children`. A `captured` flag stops headings inside non-section blocks (e.g. a blockquote) from overwriting a section's title.
